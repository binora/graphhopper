...
`profiles_cch`. Embedded applications use `CCHGraphHopper` and `CCHGraphHopperConfig`; the standard server selects the
CCH adapter automatically when `profiles_cch` is configured. See [Customizable CH](./cch.md) for the node-based adapter,
admin metric recustomization endpoint, v2 edge-state turn-cost support matrix, and request parameters.
