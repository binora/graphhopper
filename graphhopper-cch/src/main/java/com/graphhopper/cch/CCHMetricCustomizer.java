...
public final class CCHMetricCustomizer {
    public CCHMetric customize(CCHTopology topology, CCHInputGraph inputGraph) {
        Objects.requireNonNull(topology, "topology");
        return customize(topology, new NodeBasedCCHMetricSource(topology, inputGraph));
    }

    public CCHMetric customize(CCHTopology topology, CCHMetricSource source) {
        Objects.requireNonNull(topology, "topology");
        ...
        if (source.getTraversalMode() != TraversalMode.NODE_BASED)
            throw new IllegalArgumentException("graphhopper-cch v1 only supports node-based metric customization");
        if (source.hasTurnCosts())
            throw new IllegalArgumentException("graphhopper-cch v1 only supports metric customization without turn costs");

        CCHMetric metric = new CCHMetric(topology.getArcs());
        initializeDirectMetrics(metric, topology, source);
        customizeLowerTriangles(metric, topology);
        return metric;
    }

    private static void initializeDirectMetrics(CCHMetric metric, CCHTopology topology, CCHMetricSource source) {
        for (int i = 0; i < source.getCandidates(); i++) {
            ...
            if (metric.directCandidateIsBetter(candidate))
                metric.setDirect(candidate);
        }
    }
